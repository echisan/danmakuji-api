...
public class AdminVideoController extends BaseController {
    ...
    @GetMapping
    public Result listVideos(@RequestParam(value = "isMatch",required = false) Byte isMatch,
...
//        }
        if (validateResult!=5){
            return getErrorResult(ResultCode.PARAM_IS_INVALID, "页码不能为0或负数 或 页大小不能为负数");
        }
        if (isMatch!=null){
            if (!isMatch.equals((byte)0) && !isMatch.equals((byte)1)){
                return getErrorResult(ResultCode.PARAM_IS_INVALID, "isMatch参数有误");
            }
            result = videoService.listVideoByIsMatch(isMatch,pageNum,pageSize);
        }else {
            result = videoService.listVideos(pageNum,pageSize);
        }
    ...
    public Result listVideosByEpId(@PathVariable(value = "epId") Integer epId,
        ...
        if (isMatch!=null){
            ...
            result = videoService.listVideoByEpIdAndIsMatch(epId,isMatch,pageNum,pageSize);
        }else {
