...
public class ReplyServiceImpl implements ReplyService {
    ...
    @Override
    public List<Replies> listEpisodeReplies(Long epId, ReplyOrderBy orderBy, Long userId, Integer pn, Integer ps) {

        // 获取当前页全部父级评论
        String sql = "select * from dm_reply" +
                " right join dm_user" +
                " on dm_reply.user_id = dm_user.user_id " +
                " where ep_id = ? and is_parent = 1 and r_status=? order by dm_reply." + orderBy.getColumnName() + " desc limit ?,?";
...
//        logger.debug("reply info list: {}", replyInfoList);
        ...
        List<Long> parentReplyIds = new ArrayList<>();
        ...
        replyInfoList.forEach(replyInfo -> {
            ...
            if (replyInfo.getReply().getIsParent().equals(ReplyConstants.IS_PARENT)) {
                // 分配子评论
                Long parentId = replyInfo.getReply().getReplyId();
    ...
    @Override
    public Map<String, Object> listEpisodeRepliesToMap(Long userId, ReplyOrderBy orderBy, Long epId, Integer pn, Integer ps) {
        ...
        data.put("replies", listEpisodeReplies(epId, orderBy, userId, pn, ps));
        return data;
    }
    ...
    class ReplyInfoMapper implements RowMapper<ReplyInfo> {
        ...
        public ReplyInfo mapRow(ResultSet resultSet, int i) throws SQLException {
            ...
            reply.setEpId(resultSet.getLong("ep_id"));
            ...
            reply.setUserId(resultSet.getLong("user_id"));
            reply.setrLike(resultSet.getInt("r_like"));
            reply.setrHate(resultSet.getInt("r_hate"));
            reply.setParentId(resultSet.getLong("parent_id"));
            ...
            userInfo.setUid(resultSet.getLong("user_id"));
...
//            replyInfo.setLikeStatus(resultSet.getByte("like_status"));
     ...
     */
    @Override
    public List<ReplyInfo> listSonRepliesByParentId(Long parentId, Long userId, Integer pn, Integer ps) {
     ...
     */
    @Override
    public Map<String, Object> listPageSonRepliesByParentId(Long parentId, Long userId, Integer pn, Integer ps) {
    ...
    @Override
    public List<ReplyInfo> addReplyInfoLikeStatus(List<ReplyInfo> replyInfos, Long userId) {
    ...
    @Override
    public ReplyInfo getReplyInfoById(Long replyId) {
    ...
    @Override
    public List<Replies> listEpisodeRepliesByEpIdAndUserId(Long epId, Long userId, Integer pn, Integer ps) {
        return null;
    }

    @Override
    public Map<Long, Long> countByReplyIds(List<Long> replyIds) {
        ...
        for (int i = 0; i < replyIds.size() - 1; i++) {
            String sql1 = sql.replace("{}", String.valueOf(replyIds.get(i)));
            String sql2 = " union all \n";
            sqlBuilder.append(sql1);
            sqlBuilder.append(sql2);
        }
        String sql1 = sql.replace("{}", String.valueOf(replyIds.get(replyIds.size() - 1)));
        sqlBuilder.append(sql1);
...
//        logger.debug("final count reply sql : \n {} ", finalSql);
        ...
        Map<Long, Long> parentCountSonMap = new HashMap<>();
        jdbcTemplate.query(finalSql, rs -> {
            parentCountSonMap.put(rs.getLong("reply_id"), rs.getLong("son_count"));
        });
    ...
    @Override
    public Long countFloorByEpId(Long epId) {
    ...
    @Override
    public Long countFloorBetweenByEpId(Long epId, Long begin, Long end) {
    ...
    @Override
    public List<ReplyInfo> listSonReplyInfoByParentIds(List<Long> parentIds) {
        ...
        for (int i = 0; i < parentIds.size() - 1; i++) {
            String id = String.valueOf(parentIds.get(i));
            String sep = " , ";
            sqlBuilder.append("'").append(id).append("'").append(sep);
        }
        String lastId = String.valueOf(parentIds.get(parentIds.size() - 1));
